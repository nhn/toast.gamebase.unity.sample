    ...
    public class EditorCoroutines
    {
        public class EditorCoroutine
        {
            ...
            public EditorCoroutine(string methodName, int ownerHash, string ownerType)
            {
                ...
                this.ownerType = ownerType;
                ownerUniqueHash = ownerHash + "_" + ownerType;
                routineUniqueHash = ownerHash + "_" + ownerType + "_" + this.methodName;
            }
        }
        ...
        private struct YieldUnityWebRequest : ICoroutineYield
        {
        ...
        private static EditorCoroutines instance = null;

        private Dictionary<string, List<EditorCoroutine>> coroutineDict = new Dictionary<string, List<EditorCoroutine>>();
        private List<List<EditorCoroutine>> tempCoroutineList = new List<List<EditorCoroutine>>();

        private Dictionary<string, Dictionary<string, EditorCoroutine>> coroutineOwnerDict =
        ...
        {
            if (coroutineDict.ContainsKey(routine.routineUniqueHash) == true)
            {
        ...
        private void GoStopAllCoroutines(object thisReference)
        {
            ...
            if (coroutineOwnerDict.ContainsKey(coroutine.ownerUniqueHash) == true)
            {
        ...
        {
            if (coroutineDict.ContainsKey(coroutine.routineUniqueHash) == false)
            {
            ...
            if (coroutineOwnerDict.ContainsKey(coroutine.ownerUniqueHash) == false)
            {
            ...
            if (coroutineOwnerDict[coroutine.ownerUniqueHash].ContainsKey(coroutine.routineUniqueHash) == false)
            {
        ...
        private void OnUpdate()
        {
            ...
            for (var i = tempCoroutineList.Count - 1; i >= 0; i--)
            {
                ...
                for (int j = coroutines.Count - 1; j >= 0; j--)
                {
                    ...
                    if (coroutine.currentYield.IsDone(deltaTime) == false)
                    {
                        continue;
                    }

                    if (MoveNext(coroutine) == false)
                    {
        ...
        {
            if (coroutine.routine.MoveNext() == true)
            {
        ...
        private static bool Process(EditorCoroutine coroutine)
        {
            ...
            {
                coroutine.currentYield = new YieldUnityWebRequest { request = (UnityWebRequest)current };
            }
